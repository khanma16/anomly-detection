    ...
    def _check_rate_limit(self) -> bool:
        ...
        with self.rate_limit_lock:
            ...
            if current_time - self.last_alert_time < self.min_alert_interval:
                logger.info(
                    f"Alert rate limited: {self.min_alert_interval}s interval not met"
                )
                return False
            ...
            if self.hourly_alert_count >= self.max_alerts_per_hour:
                logger.info(
                    f"Alert rate limited: {self.max_alerts_per_hour} alerts/hour exceeded"
                )
                return False
...
"""
    ...
    def send_anomaly_alert(self, alert_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Sending anomaly alert for row {alert_data.get('row_index', 'unknown')}"
            )
            ...
            try:
                ...
                logger.info(
                    f"Anomaly alert sent successfully (Alert #{self.alert_count})"
                )
            finally:
                if smtp_server:
                    try:
...
"""
    ...
    def get_alert_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_alert_time": (
                datetime.fromtimestamp(self.last_alert_time).isoformat()
                if self.last_alert_time > 0
                else None
            ),
            "rate_limit_active": time.time() - self.last_alert_time
