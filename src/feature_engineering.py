"""
Feature Engineering Module for Network Anomaly Detection System
===============================================================

Implements dimensionality reduction and feature selection techniques for
improving model performance and reducing computational complexity.
...
class FeatureEngineer:
    ...
    def __init__(self, config_path: str = "config.yaml"):
        ...
        self.output_dir = self.config.get("feature_engineering", {}).get(
            "output_path", "feature_analysis/"
        )
        os.makedirs(self.output_dir, exist_ok=True)

        # Store fitted transformers for reuse
        self.transformers = {}
        self.feature_info = {}
    ...
    def analyze_feature_distribution(
        self, X: pd.DataFrame, y: pd.Series = None, dataset_name: str = "dataset"
    ) -> Dict[str, Any]:
        """Analyze feature distributions, correlations, and variance patterns"""
        ...
        for col in X.columns:
            ...
            analysis["missing_values"][col] = int(X[col].isnull().sum())
        ...
        if len(numeric_cols) > 0:
            ...
            analysis["variance_analysis"] = {
                ...
                "high_variance_features": variances[
                    variances > variances.quantile(0.95)
                ].index.tolist(),
                "mean_variance": float(variances.mean()),
                "variance_distribution": {
                    k: float(v) for k, v in variances.describe().to_dict().items()
                },
            }

        # Correlation analysis (limit to 50 features for performance)
        if len(numeric_cols) > 0:
        ...
        analysis_path = os.path.join(
            self.output_dir, f"{dataset_name}_feature_analysis.json"
        )
        import json
    ...
    def apply_variance_threshold(
        ...
        """Remove features with variance below threshold to eliminate near-constant features"""
    ...
    def apply_pca(
        ...
        self.transformers[f"pca_{dataset_name}"] = {"pca": pca, "scaler": scaler}

        # Collect PCA analysis information
        pca_info = {
            "n_components": n_comp,
        ...
        # Analyze feature importance in each principal component
        for i, component in enumerate(pca.components_):
            top_features_idx = np.argsort(np.abs(component))[-10:][::-1]
            pca_info["feature_importance"][f"PC{i+1}"] = [
                {"feature": X.columns[idx], "weight": float(component[idx])}
        ...
        logger.info(
            f"PCA: {X.shape[1]} -> {n_comp} components, "
    ...
    def apply_tsne(
        ...
        if len(X) > sample_size:
            logger.info(f"Sampling {sample_size} points for t-SNE performance")
        ...
        # Create DataFrame with t-SNE component names
        ...
        logger.info(
            f"t-SNE: {X.shape[1]} -> {n_components} components, "
    ...
    def apply_umap(
        self, X: pd.DataFrame, n_components: int = 2, dataset_name: str = "dataset"
    ) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        """Apply UMAP for efficient non-linear dimensionality reduction"""
        ...
        # Create DataFrame with UMAP component names
        ...
        # Store transformer for later use
        self.transformers[f"umap_{dataset_name}"] = {"umap": reducer, "scaler": scaler}

        # Store UMAP configuration information
        umap_info = {
            "n_components": n_components,
            "n_neighbors": 15,
            "min_dist": 0.1,
    ...
    def select_best_features(
        self, X: pd.DataFrame, y: pd.Series, k: int = 20, method: str = "f_classif"
    ) -> Tuple[pd.DataFrame, List[str]]:
        """Select k best features using statistical tests for supervised learning"""
    ...
    def _visualize_pca(self, pca_info: Dict[str, Any], dataset_name: str):
        """Create PCA analysis visualization showing explained variance"""
        try:
            fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(15, 6))

            # Individual explained variance by component
            ...
            plot_path = os.path.join(
                self.output_dir, f"{dataset_name}_pca_analysis.png"
            )
    ...
    def create_dimensionality_comparison(
        self, X: pd.DataFrame, y: pd.Series = None, dataset_name: str = "dataset"
    ) -> Dict[str, Any]:
        """Compare PCA, t-SNE, and UMAP dimensionality reduction techniques"""
        ...
        try:
            ...
            logger.info("Applying PCA for comparison")
            X_pca, pca_info = self.apply_pca(
                X, n_components=2, dataset_name=dataset_name
            )
            comparison_results["pca"] = {"data": X_pca, "info": pca_info}

            logger.info("Applying t-SNE for comparison")
            X_tsne, tsne_info = self.apply_tsne(
                X, n_components=2, dataset_name=dataset_name
            )
            comparison_results["tsne"] = {"data": X_tsne, "info": tsne_info}

            logger.info("Applying UMAP for comparison")
            X_umap, umap_info = self.apply_umap(
                X, n_components=2, dataset_name=dataset_name
            )
    ...
    def _create_comparison_plot(
        self, results: Dict[str, Any], y: pd.Series, dataset_name: str
    ):
        """Create side-by-side comparison plot for dimensionality reduction techniques"""
        try:
            ...
            for i, (technique, title) in enumerate(zip(techniques, titles)):
                if technique in results:
                    ...
                    # Create scatter plot with class labels
            ...
            plot_path = os.path.join(
                self.output_dir, f"{dataset_name}_dimensionality_comparison.png"
            )
            plt.savefig(plot_path, dpi=300, bbox_inches="tight")
            plt.close()
    ...
    def save_transformers(self, dataset_name: str):
        """Save fitted transformers to disk for later reuse"""
        if not self.config.get("feature_engineering", {}).get(
            "save_transformers", True
        ):
            return

        transformers_path = os.path.join(
            self.output_dir, f"{dataset_name}_transformers.pkl"
        )
    ...
    def load_transformers(self, dataset_name: str):
        """Load previously fitted transformers from disk"""
        transformers_path = os.path.join(
            self.output_dir, f"{dataset_name}_transformers.pkl"
        )
        info_path = os.path.join(self.output_dir, f"{dataset_name}_feature_info.pkl")
    ...
    def generate_feature_report(self, dataset_name: str) -> str:
        ...
        if f"pca_{dataset_name}" in self.feature_info:
            ...
            report_lines.append(
                f"Total variance explained: {pca_info['total_variance_explained']:.4f}"
            )
            report_lines.append(
                f"Top 3 components variance: {pca_info['explained_variance_ratio'][:3]}"
            )
            report_lines.append("")
        ...
        report_path = os.path.join(
            self.output_dir, f"{dataset_name}_feature_engineering_report.txt"
        )
